#!/usr/bin/env bash
# ARCHDROID PRO ULTIMATE INSTALLER v12.1 (Kernel Fixed)
set -euo pipefail
...
check_uefi() {
    [[ -d /sys/firmware/efi ]] || {
        echo "ERROR: Se requiere sistema UEFI"
        exit 1
    }
}
...
secure_configure() {
    ...
    echo "Configurando GRUB para ${CFG[KERNEL]}..."
    if [[ "${CFG[KERNEL]}" != "linux" ]]; then
        sed -i "s/GRUB_DEFAULT=.*/GRUB_DEFAULT='Advanced options for Arch Linux>Arch Linux, with Linux ${CFG[KERNEL]}'/" /etc/default/grub
        sed -i "s/vmlinuz-linux/vmlinuz-${CFG[KERNEL]}/g" /etc/grub.d/10_linux
    fi

    # Configurar hooks para LUKS
...
main() {
    ...
    echo "   Reinicia con: systemctl reboot"
    echo "   Tiempo de instalación: $(date +%T)"
}
