#!/usr/bin/env bash
# ARCHDROID ULTIMATE INSTALLER v14.1 (Fixed Shebang)
set -euo pipefail
...
show_header() {
    ...
    echo "╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚═════╝ ╚═╝  ╚═╝╚═════╝ ╚═╝╚═════╝ "
    echo -e "${COLOR_RESET}"
    echo -e "${COLOR_HEADER}Instalador Automático de Arch Linux - Versión 14.1${COLOR_RESET}"
    echo "=============================================================="
}
...
setup_mirrors() {
    ...
    [[ ${CFG[USE_CACHYOS]} -eq 1 ]] && echo -e "\nServer = https://mirror.cachyos.org/repo/x